...
class _ShopScreenState extends State<ShopScreen> {
  ...
  List<Product> get filteredProducts {
    List<Product> results = catProductsData.where((product) {
      ...
      if (selectedNestedCategory != 'All' && product.nestedCategory != selectedNestedCategory) {
        return false;
      }
      ...
      body: SafeArea(
        child: CustomScrollView(
          ...
          slivers: [
            ...
            _buildSubCategories(),
            if (currentNestedSubSections.isNotEmpty) ...[
              const SliverToBoxAdapter(child: SizedBox(height: 10)),
              _buildNestedSubCategories(),
            ],
            const SliverToBoxAdapter(child: SizedBox(height: 16)),
            _buildCountAndSort(products.length),
            const SliverToBoxAdapter(child: SizedBox(height: 12)),
  ...
  // ── 2. Main Pet Categories (Dogs, Cats, Birds, Bunnies...) ───────────────
  ...
  // ── 3. Sub-categories (All, Food, Kitten, Toys...) ───────────────────────
  ...
  // ── 4. Nested Sub-sections (Wet Food, Dry Food...) ───────────────────────
  Widget _buildNestedSubCategories() {
    return SliverToBoxAdapter(
      child: SizedBox(
        height: 34,
        child: ListView.separated(
